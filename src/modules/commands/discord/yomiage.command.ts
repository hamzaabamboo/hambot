...
export class YomiageCommand extends BaseCommand {
  ...
  constructor(private audio: AudioService, private config: ConfigService, private http: HttpService, moduleRef: ModuleRef) {
    super();
  ...
  async flushQueue(message: DiscordMessage, player?: AudioResource) {
    ...
    this.audio
      .playAudio(message, await this.getAudio(letter),           1,
      600000)
      .then((r) => r.playStream?.on('end', () => void this.flushQueue(message, r)));
  }
  
  async getAudio(string: string) {
    try {
    const data = await lastValueFrom(this.http.post(this.config.get("VOICEVOX_SERVER") + `audio_query?speaker=0&text=${encodeURIComponent(string)}`))
    const sound = await lastValueFrom(this.http.post(this.config.get("VOICEVOX_SERVER") +  `synthesis?speaker=0`, data.data, { responseType: "stream" }))
    return sound.data
    } catch (error) {
